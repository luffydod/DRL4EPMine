...
class SACConfig(PPOConfig):
    """SAC算法的配置，继承自PPOConfig，但覆盖一些特定于SAC的参数"""

    # SAC特有的超参数
    buffer_size: int = 100000  # 经验回放缓冲区大小
    learning_starts: int = 100  # 开始学习前需要的随机动作步数
    train_freq: int = 1  # 更新网络的频率（每收集train_freq个样本）
    ...
    target_entropy: str = "auto"  # 自动调整目标熵


def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="SAC运行参数")
    ...
    try:
        ...
        env = make_vec_env(
            ...
            env_kwargs={
                ...
                "discrete_action": False,  # SAC需要连续动作空间
    ...
    try:
        ...
        if save_video:
            ...
            video_writer = cv.VideoWriter(
                f"{video_path}/simulation_sac_hd.mp4", fourcc, 30.0, (width, height)
            )
        ...
        if save_video:
            ...
            print(f"视频已保存至: {video_path}/simulation_sac_hd.mp4")
...
def test_keyboard(config, no_graph=False, save_video=False):
    try:
        ...
        if save_video:
            ...
            fourcc = cv.VideoWriter_fourcc(*"mp4v")
            video_writer = cv.VideoWriter(
                f"{video_path}/sac_keyboard_control.mp4", fourcc, 30.0, (width, height)
            )
        ...
        if save_video:
            ...
            for frame in frames:
                video_writer.write(frame)
            video_writer.release()
            print(f"视频已保存至: {video_path}/sac_keyboard_control.mp4")
...
def collect_expert_data(config, no_graph=False):
    try:
        ...
        # 创建专家数据存储目录
        expert_data_dir = "expert_data_sac"
        ...
        expert_data_path = os.path.join(
            expert_data_dir, f"expert_data_sac_{timestamp}.pkl"
        )
